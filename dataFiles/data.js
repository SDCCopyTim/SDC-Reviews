...
const writeReviews = fs.createWriteStream('oneThousandReviews.csv');
writeReviews.write('id,campgroundid,username,bodytext,profilephoto,helpful,reviewdate\n', 'utf8');

function writeOneThousandReviews(writer, encoding, callback) {
  let i = 1000;
  let id = 0;
  function write() {
    let ok = true;
    do {
      i -= 1;
      id += 1;
      const campgroundid = Math.ceil(Math.random() * 100);
...
writeOneThousandReviews(writeReviews, 'utf-8', () => {
  writeReviews.end();
})